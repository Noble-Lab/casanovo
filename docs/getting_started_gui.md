...
It wraps Casanovo's command-line sub-commands — *de novo* sequencing, database search, evaluation, and training — each as its own tab, and adds tools for exploring the results.
Configure your inputs in a form, click **Run Casanovo**, and watch the output stream live in a console.

## Installation

CasanovoGUI is designed to require **no manual setup** — you do not need to install Java, Python, or `pip`.
...
2. Launch CasanovoGUI.
   ...
   Alternatively, if you already manage Casanovo in a Conda environment, enable **"Run inside a Conda environment"** in **Settings** to use it instead.
...
- **Output directory** and **output root name** — where the result `.mztab` and log are written, and the base name used for them.
...
Click **Parameters** to fine-tune any Casanovo setting (precursor m/z tolerance, peptide length, number of beams, batch size, accelerator, and more) without editing YAML by hand.
...
- **Per-residue confidence** — double-click any peptide to open a residue-by-residue confidence track tinted by Casanovo's per-amino-acid scores, alongside the PSMs supporting that peptide.

...
def _get_model_weights(
    ...
    for fn in os.listdir(cache_dir):
        parsed = _parse_ckpt(fn, ckpt_regex)
        if parsed:
    ...
    for release in repo.get_releases():
        for asset in release.get_assets():
            parsed = _parse_ckpt(asset.name, ckpt_regex)
            if parsed:
                model_id, version = parsed
                github_ckpts.append(
...
def setup_model(
    ...
    """
    Set up model config and resolve model weights (.ckpt) path.
    ...
    ------
    Tuple[Config, Path]
        Initialized model config, local path to model weights if any
...
def _parse_ckpt(
    filename: str, ckpt_regex: re.Pattern
    ...
    Optional[Tuple[str, Tuple[int, int, int]]]
        ...
        version.
    """
    parsed_string = ckpt_regex.match(os.path.basename(filename).lower())
